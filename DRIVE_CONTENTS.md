...
| 2 | `ForenXAI_UI_Handoff.zip` | `ForenXAI_UI_Handoff/` | 155 MB | A self-contained copy of the app, corpus and tests. Rebuilt by `python my_tkinter_app/make_handoff.py`; nothing in it is authored only here. |
...
| The Ollama model | 4.7 GB | `ollama pull qwen2.5:7b`. Never a Drive file — it is a registry pull, and a copied blob store will not register. |
...
5. `cd my_tkinter_app && python smoke_test.py` — expect **63/63**.
   `python smoke_test.py --llm --ui` runs the narration and Tk checks too: **86/86**.
